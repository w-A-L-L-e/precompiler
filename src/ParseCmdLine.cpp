...
void ParseCmdLine::init(){
  ...
  fRational.clear();
  //fRational.push_back(RATIONAL); //no default setting for rational
...
bool ParseCmdLine::parse(int argsize,char *argtable[]){
        ...
        while (i < argsize ) {
            ...
            else if( arg == "-gslbasename" ){
              if( i < argsize ){
                gslBaseName = string( argtable[i++] );
                bGslBaseName = 1;
              }
              else{
                ok=0;
...
algorithm   :trivial
-----------------------------------------------------------------------------*/
bool ParseCmdLine::getGslApi() const{
  return bGslApi;
}
...
/*-----------------------------------------------------------------------------
name        :hasGslBaseName
description :returns true if gsl base name was specified
parameters  :/
return      :bool
exceptions  :/
algorithm   :trivial
-----------------------------------------------------------------------------*/
bool ParseCmdLine::hasGslBaseName() const {
  return bGslBaseName;
}

/*-----------------------------------------------------------------------------
name        :getGslBaseName
description :returns basename string
parameters  :/
return      :string
exceptions  :/
algorithm   :trivial
-----------------------------------------------------------------------------*/
string ParseCmdLine::getGslBaseName() const{
  return gslBaseName;
}
